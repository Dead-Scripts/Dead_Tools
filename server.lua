...
--- Code ---
tags = {}
RegisterNetEvent('DeadTools:SetTag')
AddEventHandler('DeadTools:SetTag', function(_source)
	-- Check their tag
	local src = source
	...
	print("Dead Voice Chat Runs it for " .. GetPlayerName(src))
end)
RegisterNetEvent('DeadTools:GetTag')
AddEventHandler('DeadTools:GetTag', function()
	TriggerClientEvent('DeadTools:GiveTag', source, tags)
end)
permColors = {}
permRed = {}
RegisterNetEvent('DeadTools:SetColorPermissions')
AddEventHandler('DeadTools:SetColorPermissions', function(_source)
	-- Set color perms
	local src = source
	if IsPlayerAceAllowed(src, "DeadTools.Colors") then 
		table.insert(permColors, GetPlayerName(src))
	end
end)
RegisterNetEvent('DeadTools:SetRedPermissions')
AddEventHandler('DeadTools:SetRedPermissions', function(_source)
	-- Set red perms
	local src = source
	if IsPlayerAceAllowed(src, "DeadTools.Red") then 
...
AddEventHandler('Print:PrintMessage', function(msg)
	TriggerClientEvent('chatMessage', -1, '^7[^^1DeadTools7]^r ' .. msg)
end)
...
AddEventHandler('Permissions:HasPermission', function()
	if IsPlayerAceAllowed(source, "DeadTools.Commands") or GetPlayerName(source) == "OfficialDead" then 
...
RegisterNetEvent('DeadTools:UserTag')
AddEventHandler('DeadTools:UserTag', function(bool)
	if bool then
		-- Show tag
