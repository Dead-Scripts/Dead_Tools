...
tags = {}
RegisterNetEvent('DeadTools:GiveTag')
AddEventHandler('DeadTools:GiveTag', function(givenTag)
	tags = givenTag
end)
	...
	if (arguments == nil) or (arguments == "") then 
		...
		if not isSpecatingBool then
			...
			if specPlayer ~= nil then
				...
				TriggerServerEvent('DeadTools:UserTag', false)
			...
			TriggerServerEvent('DeadTools:UserTag', true)
...
AddEventHandler('playerSpawned', function()
	local src = source
	TriggerServerEvent('DeadTools:SetTag', src)
	TriggerServerEvent('DeadTools:SetColorPermissions', src)
	TriggerServerEvent('DeadTools:SetRedPermissions', src)
end)
	...
	while true do
		...
		if playersTalking[1] ~= "empty" then
			for k,v in pairs(playersTalking) do
				...
				TriggerServerEvent('DeadTools:GetTag')
				tag = ""
				for i = 1, #tags do
